...
class KeyboardView: UIView {
        ...
        if touch?.phase == UITouchPhase.began {
            ...
            if let reallyTouched = touchedKeyView {
                if currentTouchStatus == EnumTouchStatus.Down {
                    ...
                    checkContinuousTap(keyView : reallyTouched, downLoc: downTouchLoc!, downTime: downTouchTime, upLoc: endTouchLoc!, upTime: Date())
    ...
    let ContinuousTap_SpeedLimit_Second : CGFloat = 0.5
    let ContinuousTap_XMovementLimit : CGFloat = 10
    let ContinuousTap_YMovementLimit : CGFloat = 10
        
    var continuousTapCount : Int = 0 
	var lastDownLoc:CGPoint = CGPoint.zero
	var lastDownTime:Date = Date()
	var lastUpLoc:CGPoint = CGPoint.zero
	var lastUpTime:Date = Date()
    var lastKeyView : KeyView? = nil

    func checkContinuousTap(keyView : KeyView , downLoc: CGPoint, downTime: Date, upLoc: CGPoint, upTime: Date) {
        ...
        if continuousTapCount > 0 {  
        	if self.lastKeyView! !== keyView { toAcceptNewTap = false }
        	...
        	else if downTime.timeIntervalSince(self.lastUpTime) > Double(ContinuousTap_SpeedLimit_Second)  { toAcceptNewTap = false }
        }
        if toAcceptNewTap {
        ...
        self.lastDownLoc = downLoc
        self.lastDownTime = downTime
        self.lastUpLoc = upLoc
        self.lastUpTime = upTime
        self.lastKeyView = keyView
