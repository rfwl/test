...
class KeyboardKeyBackground: UIView, Connectable {
    ...
    init(frame: CGRect, cornerRadius: CGFloat, underOffset: CGFloat) {
        attached = nil
    ...
    override func layoutSubviews() {
        ...
        self.generatePointsForDrawing(bounds: self.bounds)
    ...
    func generatePointsForDrawing(bounds: CGRect) {
        ...
        for i in 0 ..< self.startingPoints.count {
            ...
            let p0 = CGPoint(x: currentPoint.x + (floatXCorner), y: currentPoint.y + underOffset + (floatYCorner))
            let p1 = CGPoint(x: nextPoint.x - (floatXCorner), y: nextPoint.y + underOffset - (floatYCorner))
            
            self.segmentPoints[i] = (p0, p1)
            
            let c = CGPoint(x: p0.x - (floatYCorner), y: p0.y + (floatXCorner))
