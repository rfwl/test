...
class Commander {   
    ...
    //===================================================
    // Data Members
	static var keyboardViewController:KeyboardViewController?
    static var contentView:KeyLevelView?
    ...
    static func reportTapToolbarButton(_ name:String){
       ...
       if name == "CharacterButton" { drawKeyLevelChildren(hanziDictionary.mCharacterKeyLevel); return}
        ...
        contentView?.isRenderingMUHanzis = true
        drawKeyLevelChildren(hanziDictionary.mMostUsedKeyLevel)
        contentView?.isRenderingMUHanzis = false
        return
       }
       ...
       else if name == "CloseIMEKeyButton"  {  keyboardViewController?.closeIME(); return}
       else if name == "NextIMEKeyButton" {  keyboardViewController?.gotoNextIMe(); return}       
    ...
    static func drawKeyLevelChildren(_ keyLevel:KeyLevel){        
        if let chlds = keyLevel.mChildren {
            if chlds.count > 0 {
                ...
                drawnFavors = false                
                contentView?.drawKeyLevelArray(chlds)
                return
            }
        }
        drawDefault()
